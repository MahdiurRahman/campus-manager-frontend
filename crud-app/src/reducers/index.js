...
const studentsReducer = (oldStudentsList = students, action) => {
	switch (action.type) {
		case 'ADD_STUDENT':
			return [...oldStudentsList, action.payload];
		case 'DELETE_STUDENT':
			return oldStudentsList.filter(student => (student.id != action.payload.id))	
		default:
			return oldStudentsList
	}
}

const selectedStudentReducer = (selectedStudent = null, action) => {
